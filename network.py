...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, down=True):
        ...
        if stride != 1 or in_planes != int(self.expansion*planes):
            if down:
                self.shortcut = nn.Sequential(
                    nn.Conv2d(in_planes, int(self.expansion*planes), kernel_size=1, stride=stride, bias=False),
                    nn.BatchNorm2d(int(self.expansion*planes))
                )
            else:
                self.shortcut = nn.Sequential(
                    nn.Upsample(scale_factor=stride, mode="bilinear", align_corners=True),
                    nn.Conv2d(in_planes, int(self.expansion*planes), kernel_size=1, stride=1, bias=False),
                    nn.BatchNorm2d(int(self.expansion*planes))
                )
        self.swish = Swish()
...
class UpResNet(nn.Module):
    def __init__(self, block, num_blocks):
        ...
        self.layer1 = self._make_layer(block, 512, num_blocks[0], stride=2, down=False)
        self.layer2 = self._make_layer(block, 256, num_blocks[1], stride=2, down=False)
        self.layer3 = self._make_layer(block, 128, num_blocks[2], stride=2, down=False)
        self.layer4 = self._make_layer(block, 64, num_blocks[3], stride=2, down=False)
...
class FPN(nn.Module):
    def __init__(self, in_dims, out_dim):
        ...
        self.init_conv = nn.Conv2d(
            in_dims[-1], out_dim, kernel_size=1, stride=1, padding=0
        )
        ...
        self.lateral1 = nn.Conv2d(
            in_dims[-2], out_dim, kernel_size=1, stride=1, padding=0
        )
        self.lateral2 = nn.Conv2d(
            in_dims[-3], out_dim, kernel_size=1, stride=1, padding=0
        )
        self.lateral3 = nn.Conv2d(
            in_dims[-4], out_dim, kernel_size=1, stride=1, padding=0
        )
...
class Network(nn.Module):
    ...
    def __init__(self, type="resnet18", out_dim=256, proj_dim=256):
        super().__init__()  
        ...
        self.up1 = UpResNet(block, num_blocks)
    ...
    def forward(self, x):
        ...
        outputs.extend([F.normalize(out[-1].view(out[-1].shape[0], out[-1].shape[1], -1), p=2, dim=1), dense_fv1, mlp_fv1])
        ...
        outputs.extend([F.normalize(out[-1].view(out[-1].shape[0], out[-1].shape[1], -1)), dense_fv2, mlp_fv2])
...
class UnsupervisedWrapper(nn.Module):
    ...
    @torch.no_grad()
    def _batch_unshuffle_ddp(self, x1, dense_fv1, mlp_fv1, rec, x2, dense_fv2, mlp_fv2, indx_unshuffle):
        bs_this = x1.shape[0]
        x1_g = gather_ddp(x1)
    ...
    def compute_dense_logits(self, dense_q, dense_k, queue_dense):
        ...
        label_dense = torch.zeros((logit_dense.shape[0], logit_dense.shape[-1]), dtype=torch.long, device=dense_q.device)
    ...
    def forward(self, img_q, img_k, dist=False):
        ...
        with torch.no_grad():
            ...
            if dist:
                x1_k, dense_fv1_k, mlp_fv1_k, rec_k, x2_k, dense_fv2_k, mlp_fv2_k = self._batch_unshuffle_ddp(x1_k, dense_fv1_k, mlp_fv1_k, rec_k, x2_k, dense_fv2_k, mlp_fv2_k, indx_unshuffle)
            cosine = torch.einsum("nca,ncb->nab", x1_q, x1_k)
            pos_indx = cosine.argmax(dim=-1)
            dense_fv1_k = dense_fv1_k.gather(2, pos_indx.unsqueeze(1).expand(-1, dense_fv1_k.shape[1], -1))

            cosine = torch.einsum("nca,ncb->nab", x2_q, x2_k)
            pos_indx = cosine.argmax(dim=-1)
            dense_fv2_k = dense_fv2_k.gather(2, pos_indx.unsqueeze(1).expand(-1, dense_fv2_k.shape[1], -1))

        logit_mlp1, label_mlp1 = self.compute_mlp_logits(mlp_fv1_q, mlp_fv1_k, self.q1.clone().detach())
        logit_mlp2, label_mlp2 = self.compute_mlp_logits(mlp_fv2_q, mlp_fv2_k, self.q2.clone().detach())

        logit_dense1, label_dense1 = self.compute_dense_logits(dense_fv1_q, dense_fv1_k, self.q1_dense.clone().detach())
        logit_dense2, label_dense2 = self.compute_dense_logits(dense_fv2_q, dense_fv2_k, self.q2_dense.clone().detach())
        ...
        return logit_mlp1, label_mlp1, logit_mlp2, label_mlp2, logit_dense1, label_dense1, logit_dense2, label_dense2, rec_q, rec_k
